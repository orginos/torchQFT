...
def test_realNVPjacobian():
    ...
    for k in range(batch_size):
        ...
        diffs.append(np.abs(ldet - J[k].detach().numpy())/V).cpu()
...
def test_realNVP():
    ...
    rev_check = (tr.abs(zz-z)/V).mean().cpu()
    print("Should be zero if reversible: ",rev_check.detach().numpy())
    diff = o.action(x.view(x.shape[0],L,L))+flow.log_prob(x).cpu()
...
def test_RGlayer():
    ...
    print(phi2[0,:,:])
    print(rphi[0,:,:])
    rev_check = (tr.abs(rphi-phi)/V).mean().cpu()
    print("Should be zero if reversible: ",rev_check.detach().numpy())
    ...
    print(phi2[0,:,:])
    print(rphi[0,:,:])
    rev_check = (tr.abs(rphi-phi)).mean().cpu()
    print("Should be zero if reversible: ",rev_check.detach().numpy())
...
def test_MGflow_train(load_flag=False,file="mg-model.dict"):
    ...
    diff = o.action(x)+mg.log_prob(x).cpu()
    print("max  action diff: ", tr.max(diff.abs()).detach().numpy())
    print("mean action diff: ", diff.mean().detach().numpy())
    print("std  action diff: ", diff.std().detach().numpy())
