...
def gamma_method_with_replicas(data_replicas: list, f, S: float = 1.0, max_lag: int = 200):
    ...
    data_var = projected.var(unbiased=True).item()
    #var_f = acf_avg[0] * (2 * tau_int / len(projected))
    var_f = data_var * (2 * tau_int / len(projected))
...
def gamma_method_with_replicas_corregido(data_replicas: list, f, S: float = 1.0, max_lag: int = 200):
    all_data = tr.cat(data_replicas, dim=0)
    mean_a = all_data.mean(dim=0)
    
    # AGREGAMOS gm. a las funciones ayudantes
    grad_f = compute_gradient_autograd(f, mean_a)
    projected = tr.cat([project_observable(rep, grad_f) for rep in data_replicas])
    ...
    # 1. Varianza real de los datos
    data_var = projected.var(unbiased=True).item()

    # 2. Protegemos contra un tau_int negativo por exceso de ruido
    safe_tau_int = max(tau_int, 0.5) 
    
    var_f = data_var * (2 * safe_tau_int / len(projected))
    
    # 3. Protegemos contra raíces negativas o cero exacto
    var_f = abs(var_f) + 1e-15
    dvalue = math.sqrt(var_f)
    
    ddvalue = 0.5 * dvalue * dtau_int / safe_tau_int
    value = f(mean_a).item()
    ...
    return {
        "acf": acf_avg, "acf_error": acf_err, "tau_int": safe_tau_int,
        "dtau_int": dtau_int, "tau_int_history": tau_hist, "dtau_int_history": dtau_hist,
        "W_hist": W_hist, "W_opt": W_opt, "value": value, "dvalue": dvalue,
        "ddvalue": ddvalue, "Q": Q
    }
